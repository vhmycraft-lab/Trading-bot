...
**Spec version:** 1.4 (2026-09-09) · **Companion documents:** `QuantLab_Implementation_Plan.md` (rationale), `docs/EVOLUTION.md` (evolutionary optimiser rationale, non-normative), `docs/ENVIRONMENT.md` (hidden randomised training environments, non-normative). This file is normative; where any two disagree, this file wins.
...
| 1.4 | **The LLM layer, paper trading and the dashboard, with one extraction that made §16 provable.** (a) **§8.4's fill arithmetic moves to `core/execution.py`** — `slipped_price`, `size_from_notional`, `fee_of`, `round_to_lot`, and the rebalance deadband `needs_order` — and `adapters/engine/simple_bar.py` now calls it rather than owning it. §16.2 requires a paper session to fill "with the same fee/slippage model as backtests", and §16's acceptance criterion is stronger still: replayed trades must equal the engine's **bit for bit on quantity and price**. A second implementation cannot hold that — not through carelessness but through drift, and the drift would surface as a paper session quietly disagreeing with the backtest that authorised it, which is the one comparison paper trading exists to make. Two things were learned doing it: floating-point multiplication does not associate, so `fee_of` keeps the engine's original grouping and the golden baselines of §20 caught the regrouped form immediately; and the **deadband is part of the strategy, not an optimisation** — without it a held position is re-sized on every bar, because a fraction computed from a moving mark price is never the fraction it was, and a strategy that entered once and held for a month would post hundreds of trades and lose money doing nothing. (b) **§16.1 gains `ports/broker.py`**, and INV-1 is stated there as a property of the port's *shape*: `Broker` exposes submit, on_bar_open, position and equity, and there is no credential, endpoint, network or async call to point at an exchange. `PaperBroker` is the only implementation; `MarketDataFeed` yields **closed bars only**, because a feed emitting the forming bar would hand a live strategy the one thing INV-3 denies a backtested one. (c) **§16.2's resume guarantee is tested the way §13.9's is**: one session run straight through against one interrupted mid-stream, required to be indistinguishable — position, cash, fees and the pending order that had not yet filled. Restoring the position and skipping to live bars is faster, looks correct, and silently produces a different account. (d) **§12's LLM layer lands mock-first**: `ports/llm.py`, `MockLLMProvider`, then `GLMProvider` tested against a fake transport. **No `GLM_API_KEY` exists and none is needed** — what has to be asserted is the adapter's policy (the daily cap refuses *before* sending, an unpriced model is refused rather than charged zero because zero makes the cap unreachable, a 4xx that is not 429 is not retried, the key lives on no attribute a traceback would print), and all of it is observable from the requests the adapter makes. (e) **INV-6 and INV-11 stop being unenforced.** `tests/unit/test_redaction.py` — named in the invariant table since 1.1 and absent until now — exists, and `research/redaction.py` builds reports from a whitelist *and* scans them. **Amendment 1.3(f) is implemented here**: the scan also covers secrets and the process environment, not only test-segment data. Prompts are written to `artifacts/llm/<id>/prompt.json` unencrypted and kept, so a report assembled by formatting an exception or a config dump carries an API key into a file that outlives the run. The partition scan matches a partition word **paired with a quantity word**, not the word alone: the system prompt has to be able to say "you will never see the test period" without tripping its own guard, and every shipped template is asserted to pass the scan it is subject to. (f) **§22's T42 acceptance criterion — "a grep shows no write calls to the store" — is a test**, over the AST rather than the text, and the list of write methods is checked against the port so a method added later cannot pass silently. The dashboard goes through `build_container(profile="research")` rather than opening an engine, since a viewer that could build one could build it without INV-5's guard. The family tree is built from persisted lineage only, and a candidate the store holds no fitness for is shown as **unmeasured**, never as zero — zero is a score, and on screen an invented number is indistinguishable from an earned one. |
| 1.3 | **The multiple-testing count, and two gates that were passing on stale evidence.** Found by audit against the working tree, not by a failing test — each defect looked like success. (a) **§14.4 check 1's `M` was not the search size.** `cli/validate.py` computed `trials = max(1, n_bars // 100)` — a function of how long the *validation segment* is — while §14.4 defines `M` as `evolution_run.n_evaluations + optuna n_trials + family.validation_touches`. The two are unrelated quantities, and the substitute is systematically the smaller: a three-generation campaign at population 32 charges `M = 96` where the bar count charged 6, and on a real curve that is the difference between `DSR = 0.95` (check 1 passes, 0 points) and `DSR = 0.89` (check 1 fails, +25). Every verdict computed that way is **overstated**, and the direction of the error is always towards `CANDIDATE`. `ExperimentStore` gains **`search_trials_for_family`**, which assembles the three terms by joining `strategy_version -> candidate -> evolution_run`, `optuna_study` and `strategy_family`; `_deflated` no longer has the result object in scope to derive `M` from, and `tests/unit/test_deflated_sharpe.py` asserts its signature is exactly `(result, n_trials)` so the bar count cannot come back. (b) **A verdict now records which formula produced its `M`** (`validation_verdict.m_formula_version`, migration `0004`, default `'m1-bar-count'`). §6 forbids deletion and rewriting, and a stored verdict may already have been cited, so the superseded ones are **marked, not corrected**: the migration stamps every pre-existing row as computed under the old formula and nothing is recomputed in place. Re-validating writes a new verdict, as it always did. **The lockbox refuses a verdict it did not compute under the current formula** (`m2-search-size`), and refuses one that names no formula at all — a verdict of unknown provenance is not authorisation. This is the one place the marking has to bite: a family gets one look at the test partition ever (§14.6) and a `LOCKBOX_FAIL` closes it permanently, so admitting on arithmetic already known to be wrong would spend an irreversible budget on overstated evidence. (c) **§19's coverage gate now checks the report's provenance before its thresholds.** `scripts/check_coverage.py` read whatever `coverage.json` was on disk: a report from an earlier session, or a hand-written one, produced a clean green pass naming modules that no longer exist. It now refuses a report older than one hour, one that will not say when it was written, one measured without branch coverage (the thresholds sum statements *and* branches, so a line-only report describes an easier quantity under the same numbers), and one naming files that are not in the tree. `meta.timestamp` carries no offset and is read as **local** time, which is what coverage.py writes. (d) **§6's `candidates_for` orders by `(gen_index, candidate_id)`, not by `created_at`.** Wall-clock order is not deterministic — rows written inside the same millisecond tie, and the tie broke differently on replay — so INV-7's "a resumed run produces the identical `candidate_id` sequence" held only by luck. (e) The general resume test is **parameterised over both environment modes** rather than leaving the environment-aware half of INV-9 to `test_environment_evolution.py` alone: a guarantee asserted in exactly one file is one refactor away from being asserted in none. |
| 1.2 | **Project Rome**: hidden randomised training environments, and the active population fixed at 32. Adopted from the Project Rome security and anti-overfitting specification, which supersedes this document where the two disagree. (a) New **§4a Training environments**: every generation is evaluated inside an environment — a window of the *training* segment, a starting capital, a slippage level, an asset subset — selected by `core/environment.py` from a seed drawn per generation from the operating system's CSPRNG and expanded with HMAC-SHA256. Commission (Rome §12), execution timing (§13) and the slippage *model* (§39) are **not** randomised, and `TrainingEnvironment` has no field for them. (b) The pool of valid windows is enumerated from the training segment *before* any draw, so a window that could reach validation or test is never a candidate — validity is a property of the enumeration, not of a check. (c) **§6 gains `training_environment`** (migration `0003`): one append-only row per generation carrying the seed, window, capital, slippage, universe and model versions, `UNIQUE (evolution_id, gen_index)`, written **before** the generation runs. The seed lives here and nowhere else. (d) **INV-7's evolution clause is restated**: a run replayed from its stored seed *and its stored environments* produces the identical `candidate_id` sequence. Resume reads the recorded environment rather than drawing, which is the only reading compatible with per-generation randomisation; drawing again would make every resumed run a different search wearing the same run id. (e) A run's identity (§11.2) does not include the bar range, so the environment id qualifies the **segment** (`train:<id>`). Without that, two generations drawing different windows but equal capital and slippage would share a run id and the second would be served the first's results. The id is `sha256(seed)[:16]` — opaque, per Rome §34. (f) A survivor carried forward is therefore **no longer a cache hit**: it is re-evaluated on the new generation's history, which is the intent and which raises the evaluation count a campaign charges into §14.4's `M`. (g) **§13.1's population becomes 32** (Rome §1), with `n_survivors`/`n_offspring`/`n_immigrants` at 24/6/2 — the same proportions scaled. The existing sum identity already makes "no cycle finishes with 31 or 33" a property of the configuration. (h) `ExperimentStore` gains `record_training_environment`, `find_training_environment` and `training_environments_for`; `evolution/` may not import an adapter (INV-8) and the loop consumes only an injected `environment_for` callable and a three-field value type carrying no seed. Known limitations are stated in `docs/ENVIRONMENT.md` §7 rather than left implicit. |
| 1.1.20 | §14.4 made precise while implementing scoring. (a) Check 2's two thresholds are **bands**, not cumulative charges: both read one quantity and `PBO > 0.5` implies `PBO > 0.3`, so charging both would make the higher band worth 35 rather than the 25 the table assigns it. Checks 6 and 10 each read three *different* quantities and so are cumulative. (b) A soft check whose input was never measured charges **nothing** and is recorded as unmeasured — the opposite of a hard gate, which fails without its evidence. A gate asks for evidence of safety; a check charges for evidence of fragility and has none. A `CANDIDATE` with a long unmeasured list is a strategy nobody finished checking, and the report says so. (c) Check 8 does not charge when the validation period holds only one calendar quarter: concentration in the only quarter there was is not a regime finding. (d) The validation touch of §14.1 step 8 is charged **after** the judging, so a pipeline that crashed mid-judgement does not spend a look it never used; the freeze at `family_max_validation_touches` then makes the twenty-first validation impossible rather than merely noticed. (e) `SensitivityReport` moves to `core/validation/sensitivity.py`, the file §14.4 names, and the neighbourhood behind it is still computed once in `optimize/plateau.py`. (f) `ExperimentStore` gains `find_family`, `record_verdict` and `verdicts_for`; a verdict is append-only, so re-validating writes a new one and the two together are the history of how the platform's opinion changed. |
| 1.1.19 | §14.3 and §14.4 made precise while implementing the statistical layer. (a) `validation.sanity_drift_allowance` is added to §5: §14.3 names it and gives its default (0.5) but no key held it. (b) **Every gate treats an absent measurement as a failure**, not as a pass: a probe that never ran, a cost stress never measured, a permutation test never computed and a missing buy-and-hold baseline each fail their gate. "Not checked" and "checked and clean" are different states and only one is evidence. (c) `G_DEGRADE` applies only its sign test when the *train* Sharpe is not positive: dividing by it would let a strategy that lost money on both segments pass by losing slightly less on one. (d) The deflated Sharpe ratio takes a **per-period** Sharpe, since `T` counts bars; `deannualise` converts `MetricSet.sharpe`, and passing the annualised figure with a bar count returns 1.0 for everything with no exception. `kurtosis` is the **non-excess** fourth moment. (e) CSCV **samples** its splits when there are more than `max_combinations`; a lexicographic prefix puts the early blocks in-sample in every split, which made PBO on pure noise range from 0.01 to 0.82 across five seeds instead of centring on 0.5. (f) A permutation the strategy could not be scored on is **counted, not dropped** — shrinking the denominator would make the p-value look more significant the more often the strategy failed — and `p = (1 + beaten) / (1 + n)` is never zero, because a finite number of resamples cannot support that claim. |
| 1.1.18 | §15 made precise while implementing the walk-forward. (a) The **optimiser is injected**: §13.8's Optuna study and §13.2's evolution are both valid searches for a window, and INV-8 forbids `walkforward/` from importing the latter. A `WindowSearch` reports its own `n_evaluations`, because only the search knows whether a repeated parameter set was served from the run cache. (b) Stitching **drops each window's first equity point**, which is the previous window's last by construction; keeping it would double-count a bar. (c) The stitched metrics are computed from a synthetic `BacktestResult` through `core/metrics.py`, so there is one metric implementation rather than a second that computes "walk-forward Sharpe" slightly differently; the windows' trades are carried across and renumbered, since the stitched account took all of them. (d) Parameter stability is measured for **numeric parameters only**, and a parameter whose coefficient of variation is undefined is omitted rather than reported as zero — a categorical has no CV, and reporting zero would read as perfect stability. (e) `WalkForwardResult.evidence` states in words that a fixed-parameter run did not test whether the search generalises, so §15.2's warning cannot be lost between the runner and the report. |
| 1.1.17 | §13.7 made precise while implementing promotion. (a) `PROMOTED_SEGMENTS` is `('val',)` and nothing else: a promotion authorises a validation run, and the test partition stays a separate door with its own record (§14.6). `promote(segment='test')` raises rather than being merely unusual. (b) Validation touches are counted **per family**, not per strategy version: §14.1's freeze at twenty is about how often one *idea* has been measured against the validation set, and a re-parameterised version of it is the same idea. (c) The similarity filter of §13.7 runs against the candidates *already chosen*, walking the ranking downwards — the same shape as §13.5's niching, and for the same reason. Rebuilt from the store it has only the structural half available (position series are not stored, §13.5), which errs towards promoting fewer and more clearly distinct candidates. (d) `ExperimentStore.promotions_for` joins the port: `require_promotion` is what a validation run consults, and `evolution/` may not import an adapter (INV-8). |
| 1.1.16 | §13.2 and §13.9 corrected while implementing the loop. (a) A generation's row is written **before** its candidates, not after: `candidate.generation_id` is a foreign key (§6), so the generation must exist first, while §6 also makes a generation row append-only with statistics known only once every candidate is scored. Both are satisfied by writing the whole generation — row, candidates, mutations, scores — together once it is complete. §13.9's "resume re-enters at the first generation with no `generation` row" therefore still holds exactly. (b) **Resume recomputes every generation from zero and records only the missing ones.** A generation's shape depends on the previous generation's *plan*, and niching reads position series the store does not keep (only their digest, §13.5), so re-entering from a partial reconstruction plans differently and silently becomes a different search — observed, then fixed. Recomputation is cheap and is what §13.9 already promises: every evaluation of a recorded generation is a cache hit, so nothing is re-executed. Replayed generations are not re-counted into `n_evaluations`. (c) A candidate whose *run* fails is rejected with the error **type recorded on the run**, since the runner files a failed run rather than raising; only `EngineError` propagates, because it is a defect in the platform rather than in the candidate. (d) `ExperimentStore` gains `find_evolution_run`, `metrics_for` and `trades_for`: `evolution/` may not import an adapter (INV-8), and a resumed run must read its population size and seed from the run as opened rather than from today's configuration. |
| 1.1.15 | §13.8 made precise while implementing refinement. (a) `sortino_dd` is `sortino - dd_lambda * max_drawdown`: the ratio already divides by downside deviation, and the drawdown term prices the *path*, which a ratio of averages cannot see. (b) The `min_trades` guard and an undefined metric are **rejections**, not penalties — a search allowed to maximise metrics estimated from three trades would find those three trades. `REJECTED` sits far below any attainable score so a sampler learns to avoid the region instead of being handed a `None`. (c) Plateau selection ranks on the **median neighbour**, not the centre, and keeps the point optimum on a tie: moving on a lexicographic tie-break would have selection wander away from the trial the search ranked highest for no reason a reader could follow. (d) The `grid` sampler is refused: it needs an explicit search space, and this study builds one from the strategy's own `ParamSpec` bounds. (e) `optuna_study.storage_path` is empty by design — the study runs in memory and §6 owns the record, so an Optuna database as well would be a second answer to "what was searched". (f) `ExperimentStore` gains `record_optuna_study`, which `cli/optimize.py` needs and the port did not declare. |
| 1.1.14 | §13.2 and §13.6 made precise while implementing population assembly and lineage. (a) Parent selection is weighted by **rank**, not by fitness. Fitness is a score in `[0, 1]` whose spread varies wildly between generations, so weighting by it would let the best survivor monopolise an early generation and count for almost nothing in a converged one; rank weights have the same shape every time. (b) A generation whose survivor set is *empty* — everything rejected — refills entirely with immigrants rather than failing. §13.2 step 4 already turns a partial shortfall into immigrants; extending that to a total one is what keeps one bad generation from ending a run. (c) `ExperimentStore` gains `generations_for` and `mutations_for`: `evolution/lineage.py` may not import an adapter (INV-8), and INV-10's replay needs the recorded edits. (d) `verify_run` **skips seeds and opaque candidates with a stated reason** rather than passing them silently — a report that counted them as verified would let a run of nothing but seeds claim a clean bill of health. (e) `ancestry` ends with the candidate itself and `descendants` does not include it; each reads the way its name does. |
| 1.1.13 | §13.5 made precise while implementing diversity. (a) `agreement` excludes bars on which **both candidates are flat**. §13.5 names the quantity it wants — strategies that "enter and exit together" — and agreeing to stay out of the market is not that. Counting mutual inactivity puts a floor under the measure that rises with how selective the two strategies are: measured on this repository's own baselines, `sma_cross` against `rsi_reversion` scored **0.66** under the literal reading and **0.03** under this one, and §22's acceptance criterion ("below 0.2 for two unrelated baselines") is unreachable under the former. (b) `buy_and_hold` against `random_entry` is the one baseline pair that stays above 0.2, at 0.27, and legitimately so: `random_entry` is long on ~45 % of bars and `buy_and_hold` on all of them. (c) The multiset Jaccard of §13.5 is the **generalised** form, `Σ min / Σ max`, which is the ordinary index on sets and keeps the information that three moving averages is not two. Risk controls join the same multiset rather than being scored separately, because §13.5 gives one Jaccard over the triples *plus* the controls. (d) `behaviour_hash` is magnitude-sensitive while `agreement` is sign-based, so an equal hash implies perfect agreement but not the reverse. That is the correct direction for a cheap duplicate pre-filter: missing a pair costs one comparison, while claiming a false duplicate would discard a distinct candidate. (e) Two candidates with no position series yet are taken as behaviourally *identical*, so niching falls back to structure alone; assuming they differ would be an unearned claim to diversity. |
| 1.1.12 | §13.4 made precise while implementing mutation. (a) The **operator library** §13.4 refers to is `evolution/library.py`, and it draws comparisons between operands measured in the same units: `IndicatorSignature` gains a `domain`, and a constant is only ever drawn against a scale-free quantity, where a fixed level means something. Genome *validity* is unchanged and stays looser — comparing a moving average against a fixed price level is a legitimate strategy a person may write — because the library bounds what the *search* draws, not what the type permits. `replace_indicator`'s "compatible signature" is read the same way: same domain, and a minimum period the current one satisfies. (b) A mutation record stores the **resulting value at its path**, not the recipe that produced it, so INV-10's replay is a pure application of edits and never re-draws; `rng_seed` is still recorded, as §13.4 requires, but replay does not need it. (c) A parameter operator is drawn only from those that can act on the candidate: a genome's parameters are numeric by construction (§9.6), so drawing `toggle_bool` for one would spend the repair budget of layer 2 on the impossible. This narrows which operator is *drawn*, never what an operator may do. (d) Warm-up is **recomputed** after a structural edit. Rule 6 is a lower bound, so raising warm-up to cover a longer lookback is not repairing by relaxing a rule — it is strictly more conservative, and the alternative is discarding otherwise sound children. (e) An immigrant genome derives its warm-up rather than drawing it, for the same reason. |
| 1.1.11 | §13.3 made precise while implementing fitness. (a) `InnerFoldReport` and `SensitivityReport` are defined in `core/fitness.py`: fitness consumes them and lives in `core`, which may not import `evolution` or `optimize` (INV-8), so the types belong to the consumer and T50/T51 fill them in. `InnerFoldReport` carries per-fold numbers and derives the three aggregates §13.3 names — mean IS Sortino, mean OOS Sortino, and the CV of per-fold OOS return — in one place. (b) "Linear decay" for `p_instability` and `p_sensitivity` reaches 0 at **twice** the threshold: §13.3 names no endpoint, and twice is the one choice that needs no second knob and cannot be configured inconsistently with the threshold it decays from. (c) An **unmeasured** input scores 0 in its component but is **not** also penalised: `inner_oos` is 0 without an inner walk-forward, while `p_divergence` and `p_instability` stay 1.0, because charging twice would rank a candidate nobody measured below one measured and found divergent. (d) `p_removal` takes the **minimum** across removal depths, as §13.3 writes it: a strategy that survives losing its best trade but collapses on losing its best five is concentrated, and averaging that away is the failure the test exists to catch. (e) The gates report the **first** failure in §13.3's tabulated order, not all of them: a rejected candidate is discarded, so the reason only has to be one and the same one every time. |
| 1.1.10 | §9.6 made precise while pulling the genome forward, so T20's `kind='genome'` path has one canonical representation and one deterministic compilation path rather than a placeholder. (a) Three **narrowings**, each fail-closed and each shrinking the reachable genome space rather than widening it: an operand may not name `bbands`, `donchian` or `macd`, because they return named tuples and `kwargs` — "kwargs match that indicator's signature" — has nowhere to put a field selector; a genome parameter must be `int` or `float`, since every one of them ends up on a side of a numeric comparison; and a compiled genome is long-or-flat, direction being an axis §9.6 gives the genome no field for. `random_entry` therefore has no genome and stays an `opaque` candidate. (b) Rule 6's "largest lookback" is costed at each parameter's **upper bound**, not its default: warm-up is declared once and the optimiser may set the period anywhere in its range. (c) Rule 5 also refuses a condition between two constants — its answer is the same on every bar, so it would occupy a slot and pay the complexity penalty for nothing. (d) `genome_id` is defined in `core/genome.py` and re-exported from `evolution/compiler.py`: it is a pure function of a core type, and `core` may not import `evolution` (INV-8). The `evolution` layer may import `strategies_io`, one way only, as `walkforward -> optimize` already does. (e) Generated source is stored under the hash of its own bytes, so `strategies/generated/` is excluded from the repository formatter: a `make format` that rewrote a generated file would change its `strategy_id` and split the run cache of §11.2. The exclude is the guarantee — matching a formatter's line-splitting for arbitrary boolean expressions is not a claim the compiler can keep. |
...
CREATE TABLE validation_verdict (
  ...
  m_formula_version TEXT NOT NULL DEFAULT 'm1-bar-count', created_at INTEGER NOT NULL
) STRICT;
...
| # | Check | Points |
|---|---|---|
| 1 | Deflated Sharpe (`deflated_sharpe.py`; Bailey & López de Prado 2014) with `M = n_trials_accounted = evolution_run.n_evaluations + optuna n_trials + family.validation_touches`, **assembled from the store by `ExperimentStore.search_trials_for_family` and passed in** (see 1.3(a)), skew/kurtosis of per-bar returns, `T` bars. `DSR < dsr_threshold` | +25 |
...
| T20 | DONE | `strategies_io/loader.py`, `strategies_io/probe.py`, `SourceStore` port + `FileSourceStore`, `ExperimentStore.get_strategy_version`. Vectorised strategies are probed at load through the sandbox (§9.1, T19). The `kind='genome'` path is `evolution/candidates.register_genome`: compile once, then in through the same loader, so generated code takes no shortcut past the AST check or the source store |
...
| T23 | DONE | `reporting/{markdown,tearsheet}.py`, `cli/report.py` (`report show`, `report reproduce`), `__main__.py`, `tests/integration/{conftest,test_backtest_cli,test_reproduce}.py`. INV-7 is checked on a golden run and again in a fresh interpreter |
...
| T27 | DONE | `core/validation/gates.py`; `validation.sanity_drift_allowance` added to §5 |
| T28 | DONE | `core/validation/deflated_sharpe.py` |
| T29 | DONE | `core/validation/pbo.py` |
| T30 | DONE | `core/validation/permutation.py` |
| T31 | DONE | `core/validation/{sensitivity,score,pipeline}.py`, `cli/validate.py`, `docs/VALIDATION.md`; `ExperimentStore.{find_family,record_verdict,verdicts_for}`. Soft checks 10 and 11 included; DSR `M` counts every evaluation that led to the verdict |
...
| T38–T41 | unchanged | — |
| T42 | AMENDED — **scope fixed by the operator** | An **interactive** lineage/family-tree view. Every node navigates to its parents and its descendants; selecting one exposes its genome, generation, mutation/proposal origin, fitness metrics, validation status, backtests and full lineage. Built **from the persisted lineage records** (`candidate`, `mutation`, `candidate_promotion`, `validation_verdict`) and never inferred from strategy names. Zoom, pan, per-generation filtering, and selection of a strategy to inspect its complete ancestry and descendants. Supersedes T56, which was the read-only export |
...
| T46 | PARTIAL — genome and compiler done | `core/genome.py`, `evolution/{compiler,candidates}.py`, `tests/unit/{test_genome,test_compiler,test_candidates}.py`. Pulled forward of T45+ so T20 has one canonical representation. **Remaining:** the operator library that draws a random genome (T49) |
| T47 | DONE | `core/fitness.py`, `core/validation/concentration.py`; `MetricSet` already carried `consistency` and `retention_*` (T11). The trade statistics `compute_metrics` computes inline are extracted so the removal test recomputes them with the same rule |
| T48 | DONE | `evolution/{library,mutation}.py`; `IndicatorSignature.domain` in `core/genome.py`. `replay()` lands here rather than in T50's `lineage.py`: INV-10 is a property of the mutation record, and it is cheapest to establish where the record is produced |
| T49 | DONE | `evolution/diversity.py`: signatures for both candidate kinds (a genome's from its tree, an opaque one's from its AST, and deliberately comparable), similarity, niching, the diversity floor and the immigrant boost |
| T50 | DONE | `evolution/{population,lineage}.py`; `ExperimentStore.generations_for` and `.mutations_for`. Alembic `0002` and the store methods landed with the T21 amendment. INV-10 is checked over a stored five-generation run built by real mutation, and a tampered genome is shown to fail it |
| T51 | DONE | `optimize/{objectives,study,plateau}.py`, `cli/optimize.py`, `ExperimentStore.record_optuna_study`. The plateau neighbourhood is computed once and feeds §13.3's `p_sensitivity` as well as §14.4's soft check 4 |
| T52 | DONE | `evolution/{loop,inner}.py`, `cli/evolve.py` (`run`, `resume`, `status`, `lineage`), `ExperimentStore.{find_evolution_run,metrics_for,trades_for}`. INV-9 asserted at the one place a run is created, and demonstrated to fail with the assertion patched out |
| T53 | DONE | `evolution/promotion.py`, `evolve promote`, `ExperimentStore.promotions_for`. The promotion row is written before the run it authorises, and `require_promotion` refuses a validation run without one |
| T54 | PARTIAL — runner done | `walkforward/runner.py`: windows, per-window search, stitching, WFE, parameter stability, the §15.5 document. **Remaining:** `cli/walkforward.py` and `test_optimize_walkforward.py`, which need the validation experiment of phase G |
| T55–T56 | NEW | phase F′, below |
...
AC: `test_genome.py`, `test_compiler.py`; compiling the same genome twice is byte-identical; compiled output passes `ast_check` and the leakage probe; every `strategies/baselines/*` has an equivalent genome that compiles to a behaviourally identical strategy (equal `position_frac` on the fixture) — except `random_entry`, which draws from `ctx.rng` and stays an `opaque` candidate (v1.1.10).
